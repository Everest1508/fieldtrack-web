...
<script>
	...
	export default ({
		...
		mounted() {
			// Try to get existing FCM token if permission is already granted
			// Don't request permission automatically - user must click button
			if (localStorage.getItem('access_token') && messagingService.isSupported()) {
				messagingService.tryGetExistingToken().then((token) => {
					if (token) {
						console.log('Firebase messaging initialized with existing token')
					} else {
						console.log('Notification permission not granted. User can enable it via the header button.')
					}
				})
			}
