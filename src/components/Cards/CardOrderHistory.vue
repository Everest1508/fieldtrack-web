<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{paddingTop: '12px',}">
		<template #title>
			<h6>Recent Leads</h6>			
			<p>Latest lead activities</p>	
		</template>
		<a-timeline :reverse="timelineReverse">
			<a-timeline-item 
				v-for="(lead, index) in ordersData" 
				:key="index"
				:color="getTimelineColor(lead.status)"
			>
				{{ lead.title }}
				<p>{{ lead.time }}</p>
			</a-timeline-item>
		...
		<a-button type="primary" block size="small" @click="timelineReverse = ! timelineReverse">
			...
			{{ timelineReverse ? 'NORMAL' : 'REVERSE' }}
		</a-button>
	</a-card>
...
<script>
	...
	export default ({
		...
		data() {
			return {
				// Whether or not the timeline is reversed.
